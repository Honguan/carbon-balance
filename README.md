...
1. 至環境部環境資料開放平臺申請 API Key。
2. 在 `.env` 加入 `MOENV_API_KEY=取得的金鑰`。
3. 執行 `docker compose up -d --build`。
4. 登入後開啟「係數資料庫」，按「立即同步更新」。

同步來源為環境部資料集 `CFP_P_02`。下載資料只會建立係數草稿，需完成審查與發布後才會出現在五階段盤查的係數查詢中。已發布版本與歷史計算不會被覆寫。
